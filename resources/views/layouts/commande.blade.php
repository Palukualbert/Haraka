    ...
    <script>
        ...
        function showRoute(startCoords, endCoords, startAddress, endAddress) {
            ...
            window.routeControl = L.Routing.control({
                ...
                show: false, // Désactiver la boîte de dialogue par défaut
                addWaypoints: false, // Empêcher l'ajout de waypoints via l'interface
                draggableWaypoints: false, // Empêcher le déplacement des waypoints
            }).on('routesfound', function(e) {
                const routes = e.routes;
                const summary = routes[0].summary; // On prend le premier itinéraire trouvé

                // Calcul de la distance (en km) et du temps (en minutes)
                const distanceInKm = (summary.totalDistance / 1000).toFixed(2);
                const timeInMinutes = Math.round(summary.totalTime / 60);

                // Calcul du prix (1 km = 1500 FC)
                const price = distanceInKm * 1500;
        ...
        <div class="route-info card leaflet-routing-container leaflet-control leaflet-control-custom">
            <div class="card-body">
                ...
                <p><strong>Durée estimée :</strong> ${timeInMinutes} minutes</p>
                <p><strong>Prix :</strong> ${price.toFixed(2)} FC</p>
                <button class="btn btn-danger" onclick="closeRouteInfo()">Fermer</button>
            </div>
        </div>
        `;

                // Insérer la boîte de dialogue dans la carte pour qu'elle défile avec la page
        ...
        document.getElementById('orderButton').addEventListener('click', function () {
            ...
            if (start && end) {
                ...
                // Simuler l'envoi de la commande aux chauffeurs
                fetch('/api/sendNotificationToDrivers', {
                    method: 'POST',
                    headers: {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            alert('Commande acceptée par ' + data.driverName + '. Numéro de téléphone: ' + data.driverPhone + '. Position actuelle: ' + data.driverLocation);
                        } else {
                ...
                // Vérifier après 20 secondes si la commande a été acceptée
                setTimeout(() => {
                    if (!searchCompleted) {
                        document.getElementById('loadingOverlay').style.visibility = 'hidden';
                        alert('Aucun taxi disponible à proximité.');
                    }
                }, 20000);
